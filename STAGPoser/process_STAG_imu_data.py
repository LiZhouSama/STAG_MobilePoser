...
def detect_tpose_segment(lin_acc_data, ori_data, start_jump_frames=None, fps=100, min_duration=4.0, 
                        acc_stability_threshold=0.5, ori_stability_threshold=0.5):
    """
    检测T-pose段：IMU数据变化范围小且持续至少4秒的时间段，且必须在开始跳跃之后
    
    Args:
        lin_acc_data: 线性加速度数据 [N, 3]
        ori_data: 方向数据 [N, 3] (roll, pitch, yaw in degrees)
        start_jump_frames: 开始跳跃序列的帧索引，T-pose必须在此之后
        fps: 数据帧率
        min_duration: T-pose的最小持续时间（秒）
        acc_stability_threshold: 加速度稳定性阈值（m/s²）
        ori_stability_threshold: 方向稳定性阈值（度）
...
def apply_coordinate_transform(acc_data, ori_data, device_type='watch'):
    """
    ...
    Returns:
        transformed_acc, transformed_ori: 变换后的数据
    """
    # IMU坐标系到SMPL坐标系的变换矩阵
    ...
    # 转换方向数据 - 从欧拉角到旋转矩阵
    ori_tensor = torch.tensor(ori_data, dtype=torch.float32)
    N = ori_tensor.shape[0]
    transformed_ori = torch.zeros(N, 3, 3, dtype=torch.float32)
    
    for i in range(N):
        # 将欧拉角转换为旋转矩阵
        roll, pitch, yaw = np.deg2rad(ori_tensor[i])
        ...
        R_smpl = transform_matrix.numpy() @ R_imu @ transform_matrix.numpy().T
        transformed_ori[i] = torch.tensor(R_smpl, dtype=torch.float32)
...
def process_imu_files(phone_file, watch_file, output_file):
    ...
    # 提取方向数据
    phone_ori = phone_data[['ori_roll_deg', 'ori_pitch_deg', 'ori_yaw_deg']].values
    watch_ori = watch_data[['ori_roll_deg', 'ori_pitch_deg', 'ori_yaw_deg']].values
    ...
    print("\n=== 第二阶段：T-pose检测 ===")
    print("手机T-pose检测:")
    phone_tpose_start, phone_tpose_end = detect_tpose_segment(phone_lin_acc, phone_ori, phone_start_seq_temp, acc_stability_threshold=0.4, ori_stability_threshold=0.5)
    
    print("手表T-pose检测:")
    watch_tpose_start, watch_tpose_end = detect_tpose_segment(watch_lin_acc, watch_ori, watch_start_seq_temp, acc_stability_threshold=0.5, ori_stability_threshold=1.0)
...
def create_transformation_comparison_visualization(
    phone_acc_raw, watch_acc_raw, phone_ori_raw, watch_ori_raw,
    ...
    try:
        ...
        phone_euler_transformed = rotation_matrix_to_euler_degrees(phone_ori_transformed)
        phone_euler_normalized = rotation_matrix_to_euler_degrees(phone_ori_normalized)
        ...
        # === 第四行：手表方向归一化前后对比 ===
        watch_euler_transformed = rotation_matrix_to_euler_degrees(watch_ori_transformed)
        watch_euler_normalized = rotation_matrix_to_euler_degrees(watch_ori_normalized)
